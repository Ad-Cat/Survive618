...
def o(text): #等效于 print , 只是增加了暂停, 类似于 ChatGPT /0.00秒暂停/ [普通]
    for char in text:
        print(char, end='', flush=True)  # 使用 end='' 避免换行，flush=True 立即输出
        time.sleep(0.00)  # 暂停 0.00 秒
    print()  # 打印换行符，以便下一行文本能够另起一行

def o2(text):#另一版的暂停, 在day1的 捡钱部分 /0秒暂停/
    for char in text:
        print(char, end='', flush=True)
        time.sleep(0)  # 暂停 0 秒
    print()

def o3(text):#另一版的暂停, 用于 加快许可声明 /0.00秒暂停/
    for char in text:
        print(char, end='', flush=True)
        time.sleep(0.00)  # 暂停 0.00 秒
    print()

def o4(text):#另一版的暂停, 用于 加快许可证提示 /0.0000秒暂停/
    for char in text:
        print(char, end='', flush=True)
        time.sleep(0.0000)  # 暂停 0.0000 秒(怎么四位小数了())
    print()

o('\n注意')
o('极速版, 如果想体验完整速度, 请下载完整版. 此版可供体验. 使用time.sleep不会暂停')
